...
class Scanner {
  ...
  bool get _isDocumentIndicator =>
      _scanner.column == 0 && _isBlankOrEndAt(3) && (_scanner.matches('---') || _scanner.matches('...'));
  ...
  void _saveSimpleKey() {
    ...
    _simpleKeys[_simpleKeys.length - 1] = _SimpleKey(
        _tokensParsed + _tokens.length, _scanner.line, _scanner.column, _scanner.location,
        required: required);
  }
  ...
  void _fetchValue() {
    ...
    if (simpleKey != null) {
      ...
      _tokens.insert(
          simpleKey.tokenNumber - _tokensParsed, Token(TokenType.key, simpleKey.location.pointSpan() as FileSpan));

      // In the block context, we may need to add the
      // [TokenType.BLOCK_MAPPING_START] token.
      _rollIndent(simpleKey.column, TokenType.blockMappingStart, simpleKey.location,
  ...
  Token _scanBlockScalar({bool literal = false}) {
    ...
    return ScalarToken(
        _scanner.spanFrom(start, end), buffer.toString(), literal ? ScalarStyle.LITERAL : ScalarStyle.FOLDED);
  }
